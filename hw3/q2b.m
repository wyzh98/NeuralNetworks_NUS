...
for stddev = [0.1 1 stddev0 100 1000 10000]
    ...
    w = pinv(phi_train'*phi_train + lambda*eye(N)) * phi_train'*trainLabel;

    r = dist(testData, sampleData');
    ...
    h4 = title(sprintf('Std Dev = %g', stddev));
    set([h1 h2 h3 h4], 'Interpreter', 'latex');
    fprintf('Stddev: %g\nLoss train: %f\nLoss test: %f\nAcc train max: %f\nAcc test max: %f\n\n', ...
            stddev, loss_train, loss, 100*max(trainAcc), 100*max(testAcc));
end